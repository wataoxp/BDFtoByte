...
template <typename T>
std::vector<T> REGEX::StringtoHex(const std::string& file)
{
    // 文字列を<X>の型で格納する
    std::vector<T> FontData;
    // 16進数のパターンを検索
    ...
    for (; it != end; it++)
    {
        ...
        T value = static_cast<T>(std::stoul(HexStr, nullptr, 16));
...
// テンプレートの明示的なインスタンス化
template std::vector<uint8_t> REGEX::StringtoHex<uint8_t>(const std::string& file);
template std::vector<uint16_t> REGEX::StringtoHex<uint16_t>(const std::string& file);
template std::vector<uint32_t> REGEX::StringtoHex<uint32_t>(const std::string& file);
...
std::string REGEX::FontReverse(const std::vector<T>& data,const std::string& name)
{
    ...
    // Tの型を特定
    const std::type_info& DataType = typeid(T);
    ...
    for (uint32_t moji = 0; moji < allchar; moji++)
    {
        ...
        {
            // dataは一次元配列なので、(文字インデックス * 1文字あたりの行数 + 行インデックス) でアクセス
            word = data[moji * font.Width + i];
...
// テンプレートの明示的なインスタンス化
template std::string REGEX::FontReverse<uint8_t>(const std::vector<uint8_t>& data, const std::string& name);
template std::string REGEX::FontReverse<uint16_t>(const std::vector<uint16_t>& data, const std::string& name);
template std::string REGEX::FontReverse<uint32_t>(const std::vector<uint32_t>& data, const std::string& name);
